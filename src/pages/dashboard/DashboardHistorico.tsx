...
interface DashboardHistoricoProps {
...
const getPeriodoAnterior = (periodo: FiltroPeriodo, dataFimAtual: Date) => {
...
const calcularDiferencaAtualAnterior = (totalAtual: number, totalAnterior: number): { diff: number; percentual: number; sinal: Trend } => {
...
export function DashboardHistorico({ filtroPeriodo }: DashboardHistoricoProps) {
  ...
  const GraficoLinha = ({ dados }: { dados: { data: string; ocorrencias: number }[] }) => {
  ...
  const GraficoPorTipo = ({ dados }: { dados: [string, number][] }) => {
    if (!dados || dados.length === 0) {
              ...
              <div style={{ flex: 1, height: 14, background: "#f0f0f0", borderRadius: 8, overflow: "hidden" }}>
                <div
                  style={{
                    width: `${(value / max) * 100}%`,
                    height: "100%",
                    background: naturezaColors[i % naturezaColors.length],
                  }}
                />
              </div>
  ...
  const GraficoPorMunicipio = ({ dados }: { dados: Record<string, number> }) => {
        ...
        <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          ...
          <Bar dataKey="quantidade" radius={[4, 4, 0, 0]}>
            {chartData.map((entry, i) => (
              <Cell key={`cell-${entry.municipioFull}-${i}`} fill={municipioColors[i % municipioColors.length]} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
        ...
        <GridColumn weight={2}>
          <MiniGrid>
            <MetricCard
              ...
              icon={<ChartBarIcon size={20} weight="fill" color="#4C6EF5" />}
              trend={
                metricasPeriodo.total.sinal !== "neutral"
                  ? {
                      type: metricasPeriodo.total.sinal,
                      text: `${metricasPeriodo.total.sinal === "up" ? "▲" : "▼"} ${Math.abs(metricasPeriodo.total.percentual).toFixed(1)}%`,
                    }
                  : null
              }
              type="total"
            ...
            <MetricCard
              ...
              meta="status"
              icon={<BellSimpleIcon size={20} weight="fill" color="#4C6EF5" />}
              trend={
                metricasPeriodo.pendentes.sinal !== "neutral"
                  ? {
                      type: metricasPeriodo.pendentes.sinal,
                      text: `${metricasPeriodo.pendentes.sinal === "up" ? "+" : "—"} ${Math.abs(metricasPeriodo.pendentes.diff)}`,
                    }
                  : null
              }
              type="pendentes"
            ...
            <MetricCard
              ...
              icon={<FireIcon size={20} weight="fill" color="#4C6EF5" />}
              trend={null}
              type="topNatureza"
            />
          </MiniGrid>
